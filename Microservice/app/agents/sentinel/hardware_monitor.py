...
class HardwareMonitor:
    ...
    async def _get_temperature_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # CPU temperature
            try:
                cpu_temps = self._wmi_client.query("SELECT * FROM MSAcpi_ThermalZoneTemperature")
                for temp in cpu_temps:
                    # Convert from tenths of kelvin to celsius
                    temp_celsius = (temp.CurrentTemperature - 2732) / 10.0
                    zone_name = getattr(temp, 'InstanceName', 'Unknown')
                    temperatures[zone_name] = round(temp_celsius, 2)
            except Exception as e:
                logger.debug(f"[Sentinel] CPU temp query failed: {e}")
            
            # GPU temperature (NVIDIA)
            try:
                # Try to get GPU temperature via WMI
                gpu_temps = self._wmi_client.query("SELECT * FROM Win32_VideoController")
                for gpu in gpu_temps:
                    gpu_name = getattr(gpu, 'Name', 'Unknown GPU')
                    # Some GPUs report temperature through adapter info
                    temperatures[f"GPU_{gpu_name}"] = "N/A"
            except Exception as e:
                logger.debug(f"[Sentinel] GPU temp query failed: {e}")
