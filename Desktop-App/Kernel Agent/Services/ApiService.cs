...
namespace Kernel_Agent.Services
{
    public class ApiService
    {
        ...
        public async Task<SentinelHealthReport?> GetSentinelHealthReportAsync()
        {
            try
            {
                var response = await GetAsync<SentinelHealthReport>($"{MICROSERVICE_URL}/api/agents/sentinel/health-report");
                return response;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[API] GetSentinelHealthReport error: {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<SentinelOptimizationResult?> OptimizeForFocusAsync(string targetApp)
        {
            try
            {
                var data = new { target_app = targetApp };
                var response = await PostAsync<SentinelOptimizationResult>($"{MICROSERVICE_URL}/api/agents/sentinel/optimize-focus", data);
                return response;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[API] OptimizeForFocus error: {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<SentinelCleanupResult?> KillResourceHogsAsync(double cpuThreshold = 90.0, double memoryThreshold = 95.0)
        {
            try
            {
                var data = new { cpu_threshold = cpuThreshold, memory_threshold = memoryThreshold };
                var response = await PostAsync<SentinelCleanupResult>($"{MICROSERVICE_URL}/api/agents/sentinel/kill-hogs", data);
                return response;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[API] KillResourceHogs error: {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<SentinelCleanupResult?> CleanupGhostProcessesAsync(double idleHours = 2.0)
        {
            try
            {
                var data = new { idle_hours = idleHours };
                var response = await PostAsync<SentinelCleanupResult>($"{MICROSERVICE_URL}/api/agents/sentinel/cleanup-ghosts", data);
                return response;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[API] CleanupGhostProcesses error: {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<SentinelPowerResult?> SetPowerProfileAsync(string profile)
        {
            try
            {
                ...
                var response = await PostAsync<SentinelPowerResult>($"{MICROSERVICE_URL}/api/agents/sentinel/power-profile", data);
                return response;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[API] SetPowerProfile error: {ex.Message}");
                return null;
            }
        }

        public async Task<SentinelMetrics?> GetSentinelMetricsAsync()
        {
            try
            {
                var response = await GetAsync<SentinelMetrics>($"{MICROSERVICE_URL}/api/agents/sentinel/metrics");
                return response;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[API] GetSentinelMetrics error: {ex.Message}");
                return null;
            }
        }
    }
    ...
    // ==================== SENTINEL AGENT DATA CLASSES ====================

    public class SentinelHealthReport
    {
        public string Timestamp { get; set; } = string.Empty;
    ...
    public class SystemInfo
    {
        ...
        public int CpuCountLogical { get; set; }
    ...
    public class CpuInfo
    {
        ...
        public List<double> PercentPerCore { get; set; } = new();
        public CpuFrequency FrequencyMhz { get; set; } = new();
        public LoadAverage LoadAverage { get; set; } = new();
    }

    public class CpuFrequency
    {
        public double CurrentMhz { get; set; }
        public double? MinMhz { get; set; }
        public double? MaxMhz { get; set; }
    }
    ...
    {
        public VirtualMemory Virtual { get; set; } = new();
        public SwapMemory Swap { get; set; } = new();
    }
    ...
    {
        public bool Available { get; set; }
        public double ReadBytesMb { get; set; }
        public double WriteBytesMb { get; set; }
    ...
    public class NetworkInfo
    {
        ...
        public double BytesRecvMb { get; set; }
        public int PacketsSent { get; set; }
        public int PacketsRecv { get; set; }
        public double SentRateMbps { get; set; }
        public double RecvRateMbps { get; set; }
    }
    ...
    public class SentinelOptimizationResult
    {
        public string TargetApp { get; set; } = string.Empty;
        public string Timestamp { get; set; } = string.Empty;
        public List<string> ActionsTaken { get; set; } = new();
        public List<ProcessAdjustment> ProcessesAdjusted { get; set; } = new();
        public bool Success { get; set; }
        public ProcessInfo? TargetProcess { get; set; }
        public string Error { get; set; } = string.Empty;
    }

    public class ProcessAdjustment
    {
        public int Pid { get; set; }
        public string Name { get; set; } = string.Empty;
    ...
    public class SentinelCleanupResult
    {
        public string Timestamp { get; set; } = string.Empty;
        public double CpuThreshold { get; set; }
        public double MemoryThreshold { get; set; }
        public List<ProcessTermination> ProcessesKilled { get; set; } = new();
        public List<ProcessTermination> ProcessesSkipped { get; set; } = new();
        public bool Success { get; set; }
        public string Error { get; set; } = string.Empty;
        public double IdleThresholdHours { get; set; }
    }
    ...
    public class SentinelPowerResult
    {
        public bool Success { get; set; }
    ...
    public class AgentStatus
    {
        ...
        public string? LastOptimization { get; set; }
    }
}
